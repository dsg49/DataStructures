...
void InterleaveOddsAndEvensInOrigOrder(Node*& headPtr)
{
   ...
   // Do not do anything if only empty, or less than three nodes in list
   if (p == 0 || p->link == 0 || p->link->link == 0)
      return;
   ...
   // Return if list is either all even or odd numbers
   if (evenCount == 0 || oddCount == 0)
      return;

   // Only while list is valid
   while (current->link)
   {

      Node* afterCurr = current->link;

      Node* prev = current;

      // Case 1: current node is even and next node is also even
      if (current->data % 2 == 0 && afterCurr->data % 2 == 0)
      {
         // Look for the first odd node (while staying within list)
         while (afterCurr->data % 2 == 0 && afterCurr->link != 0)
         {
            prev = afterCurr;

            afterCurr = afterCurr->link;
         }

         // If an odd node is found, break links and move odd node
         if (afterCurr->data % 2 != 0)
         {
            prev->link = afterCurr->link;

            afterCurr->link = current->link;

            current->link = afterCurr;
         }
      }
      // Case 2: current node is odd and next node is also odd
      else if (current->data % 2 == 1 && afterCurr->data %2 == 1)
      {
         // Look for the first even node (while staying within list
         while (afterCurr->data % 2 == 1 && afterCurr->link != 0)
         {
            prev = afterCurr;
         ...
         // If an even node is found, break links and move even node
         if (afterCurr->data % 2 == 0)
         {
            prev->link = afterCurr->link;
