...
using namespace std;
using namespace CS3358ssII15Assign04_sequenceOfNum;
...
template <class S>
S get_Element(sequence<S>& input);
// Pre:  (none)
// Post: The user is prompted to enter a real number or a non-whitespace
//       character. The prompt is repeated until a valid input can be read. The
//       valid input read is returned. The input buffer is
   ...
   do
   {
      ...
      switch (choice)
      {
         ...
         case 'A':
            ...
            if (objectNum == 1)
            {
               numHold = get_Element(s1);
               s1.add(numHold);
               cout << numHold << " added to s1." << endl;
            }
            else
            {
               charHold = get_Element(s2);
               s2.add(charHold);
