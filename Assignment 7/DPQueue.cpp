...
namespace CS3358_SumII2015_Assign07
{
   ...
   {
      if (used == capacity)
         resize(1 + (1.5 * capacity));

      // Add entry values to end of the array, which corresponds to adding it
      // as a new rightmost leaf. This maintains the shape property.
      ...
      // Check-and-swap to maintain order property
   ...
   void p_queue::pop()
   {
      ...
      // Remove the root by copying the last element at deepest
      // level to the root and taking out the last node
      ...
      size_type indexGreatestChild = 0;

      // Reheapify
      while (!is_leaf(i) &&
            (heap[i].priority <= big_child_priority(i)))
      {
         indexGreatestChild = big_child_index(i);

         swap_with_parent(indexGreatestChild);
   ...
   //             used remains unchanged.
   {
      ...
      ItemType *nHeap = new ItemType[capacity];

      size_type i = 0;

      while (i < used)
      {
         nHeap[i].data = heap[i].data;

         nHeap[i].priority = heap[i].priority;
   ...
   //       than that of the other child, if there is one.)
   {
      ...
      if (((2*i + 2) < used) &&
          (heap[2*i + 2].priority > heap[2*i + 1].priority))
         return (2*i + 2); // right(i)
      else
         return (2*i + 1); // left(i)
   }
